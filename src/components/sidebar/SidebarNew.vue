<template>
  <div :class="['hidden lg:flex lg:flex-col transition-all duration-300 ', isSidebarCollapsed ? 'lg:w-20' : 'lg:w-72']">
    <!--# The code below manages the transition and state of the sidebar's visibility on mobile and desktop screens -->
                      ...
                      <ul role="list" class="-mx-2 space-y-1">
                        <li v-for="item in navigation" :key="item.name">
                          <a :href="item.href" :class="[item.current ? 'bg-gray-800 text-white' : 'text-gray-400 hover:bg-gray-800 hover:text-white', 'group flex gap-x-3 rounded-md p-2 text-sm font-semibold leading-6']">
                            ...
                            <!--# Conditionally show/hide the text based on collapse state -->
                      ...
                      <ul role="list" class="-mx-2 mt-2 space-y-1">
                        <li v-for="team in teams" :key="team.name">
                          <a :href="team.href" :class="[team.current ? 'bg-gray-800 text-white' : 'text-gray-400 hover:bg-gray-800 hover:text-white', 'group flex gap-x-3 rounded-md p-2 text-sm font-semibold leading-6']">
                            ...
                            <span v-if="!isSidebarCollapsed" class="truncate">{{ team.name }}</span>
                          </a>
                        </li>
                      </ul>
                    </li>
    ...
    <div class="hidden lg:flex lg:flex-col transition-all duration-300 h-full" :class="isSidebarCollapsed ? 'lg:w-20' : 'lg:w-72'">
      <!--# Sidebar component, swap this element with another sidebar if you like -->
...
const sidebarOpen = ref(false); // Controls the sidebar visibility on mobile
const sidebarStore = useSidebarStore(); // Access Sidebar Store

// Computed property to get the current collapsed state of the sidebar
